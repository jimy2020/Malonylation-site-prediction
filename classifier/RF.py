...
data_=pd.read_csv(r'C:/Users/parsian/Desktop/Combining/classifier/EFivefeature-min-max_fscore150_train.csv')
data_.drop(data_.columns['Unnamed'])
data=np.array(data_)
data=data[:,1:]
[m,n]=np.shape(data)
...
cv_clf=RandomForestClassifier(n_estimators=100)
...
yscore_sum.to_csv('yscore_EWET_RF.csv')
ytest=ytest[np.array(range(1,row)),:]
ytest_sum = pd.DataFrame(data=ytest)
ytest_sum.to_csv('ytest_EWET_RF.csv')
...
data_csv.to_csv(r'EFivefeature-z-score_RF_train.csv') 
